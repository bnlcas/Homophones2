...
%
%% Mark 3:
% Adds the capability to run analysis on grid/non_grid electrodes
% Assumes the presence of artifact files for both grid and non-grid
% electrodes and automatically generates ERP strucutures for both sets of
% data:
%
%
%% MARK 2:
...
%function [] = DataProcessingPipelineWrapper_2_3(root_dir)
...
if create_evnt_structs
    ...
    anin_chan = 3; % varies, typically 2, on EC131, this was set to 3
...
for k = 1:1
    ...
    ERPs_grid = Make_unified_ERP_struct_vargrid(evnt_dir, band_inds, false);

    ERPs_nongrid = Make_unified_ERP_struct_vargrid(evnt_dir, band_inds, true);
    ...
    is_high_z_trial = auto_flag_high_z_trials(ERPs_grid, high_z_thresh, num_chan_thresh, num_timept_thresh); % flag trials with 1 ch and 1 timept with z>10)
    ERPs_grid.is_high_z_trial = is_high_z_trial;

    is_high_z_trial = auto_flag_high_z_trials(ERPs_nongrid, high_z_thresh, num_chan_thresh, num_timept_thresh); % flag trials with 1 ch and 1 timept with z>10)
    ERPs_nongrid.is_high_z_trial = is_high_z_trial;
 
    %% Add Lexical Stats
    current_func = 'DataProcessingPipelineWrapper_3_1.m';
    ...
    ERPs_nongrid = append_linguistic_stats(ERPs_nongrid, lex_stats_dir);
    
    ERPs_grid = append_linguistic_stats(ERPs_grid, lex_stats_dir);
    ...
    ERPs_grid.grd = grid_subplot_layout('br','y', 'plot_test_grid', true);
    ERPs_nongrid.grd = 1:size(ERPs_nongrid.ecog_targets,1);
    % EC123:
    ...
    %% Plot Test Data
    if plot_test_erps
        is_good = (ERPs_grid.is_good_trial == 1);    % Trials with no bad time segments
        is_sub = (ERPs_grid.is_related_subordinant == 1) & is_good;  % subordinate sense primed
        is_dom = (ERPs_grid.is_related_dominant == 1) & is_good; % dominante sense primed
        is_rel = is_sub | is_dom;                       % Related prime
        is_unr = is_good & ~is_sub &~is_dom;            % Rando prime
        is_homophone = is_good & strcmpi(ERPs_grid.target_names,'mouse'); % target homophone is mouse

        ecog1 = ERPs_grid.ecog_targets(:,:,is_rel & is_homophone);
        ecog2 = ERPs_grid.ecog_targets(:,:,is_unr & is_homophone);
        is_sig = ttest2(ecog1, ecog2, 'Alpha', 0.01, 'Dim',3); % t-significant electrodes for pval < 0.001
        is_sig(ERPs_grid.BadChans,:) = false;
        PlotECogGrid_std_v3(ERPs_grid, [-1 1], ecog2, ecog1, 'SigPts',is_sig, 'Normalize', false, 'Title', ['Grid Electrodes in ' band_names{k}]);

        ecog1 = ERPs_nongrid.ecog_targets(:,:,is_rel & is_homophone);
        ecog2 = ERPs_nongrid.ecog_targets(:,:,is_unr & is_homophone);
        is_sig = ttest2(ecog1, ecog2, 'Alpha', 0.01, 'Dim',3); % t-significant electrodes for pval < 0.001
        is_sig(ERPs_nongrid.BadChans,:) = false;

        PlotECogGrid_std_v3(ERPs_nongrid, [-1 1], ecog2, ecog1, 'SigPts',is_sig, 'Normalize', false, 'Title', ['Non-Grid Electrodes in ' band_names{k}]);

        %% Get list of significant channels for some comparisons
        is_sig_chan_rel = find_sig_chans_homophones(ERPs_grid, is_rel, is_unr);
        is_sig_chan_rel_mouse = find_sig_chans_homophones(ERPs_grid, is_rel & is_homophone, is_unr & is_homophone);
    end
    ...
    if save_erps
        ...
        save([structure_dir filesep subj '_' EPR_name '_grid' '.mat'], 'ERPs_grid','-v7.3'); % Is freaking huge...
        save([structure_dir filesep subj '_' EPR_name '_nongrid' '.mat'], 'ERPs_nongrid','-v7.3'); % Is freaking huge...
    end
