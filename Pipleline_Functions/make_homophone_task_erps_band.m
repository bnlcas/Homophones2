function ERPs = make_homophone_task_erps_band(evnt, band_inds) %, exp_dat)
...
for i = 1:num_trials
    ...
    %% load data:
    ecog_z = load_ecog_data_bands(ecog_hg_dpath, zscore_range, band_inds); % for z-scoring 256x51
    ecog_p = load_ecog_data_bands(ecog_hg_dpath, prime_range, band_inds); % for prime 256x601
    ecog_t = load_ecog_data_bands(ecog_hg_dpath, target_range, band_inds); % for target 256x601
...
%%
function ecog = load_ecog_data_bands(dpath, time_range, band_inds)
...
if ~(exist([art_dir filesep badTimeSegments_filename], 'file') == 2)
    badTimeSegments_filename = 'badTimeSegments_grid.mat';
    warning('BadTime Segments File not Found - trying "*_grid.mat" format')
    if ~(exist([art_dir filesep badTimeSegments_filename], 'file') == 2)
        warning('BadTime Segments File not Found - error')
    end
end
badChannel_file = 'badChannels.txt';
if ~(exist([art_dir filesep badChannel_file], 'file') == 2)
    badChannel_file = 'badChannels_grid.txt';
    warning('BadChannel Segments File not Found - trying "*_grid.mat" format')
    if ~(exist([art_dir filesep badChannel_file], 'file') == 2)
        warning('BadChannel File not Found - error')
    end
end

load([art_dir filesep  badTimeSegments_filename]);
bad_chans = textread([art_dir filesep badChannel_file]);
