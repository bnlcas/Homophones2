...
PlotECogGrid_std_v2(ERPs, erp_twin, ecog(:,:,is_highFA), ecog(:,:,is_lowFA), ecog(:,:,is_null), ...
...
for k = 1:length(homophones)
    ...
    PlotECogGrid_std_v2(ERPs, erp_twin, ecog(:,:,is_highFA_homo), ecog(:,:,is_lowFA_homo), ecog(:,:,is_null_homo), ...
        'SigPts', is_sigpts, 'Title', fig_title);
...
PlotECogGrid_std_v2(ERPs, erp_twin, ecog(:,:,is_rel), ecog(:,:,is_unrel), ...
    'SigPts', is_sigpts, 'Title', fig_title);
    drawnow
    pause(5)
    set(gcf, 'PaperPositionMode', 'auto')
...
PlotECogGrid_std_v2(ERPs, erp_twin, ecog(:,:,is_dom), ecog(:,:,is_sub), ecog(:,:,is_null),...
             'SigPts', is_sigpts, 'Title', fig_title);
...
for k = 1:length(homophones)
    ...
    fig_title = ['Dominant Primes (' ERPs.prime_names{find(is_dom_homo,1)} '; Red; n = ' num2str(sum(is_dom_homo)) ') vs Subordinate Primes (' ERPs.target_names{find(is_sub_homo,1)} '; Blue; n = ' num2str(sum(is_sub_homo)) ') for ' upper(homophones{k})];

    PlotECogGrid_std_v2(ERPs, erp_twin, ecog(:,:,is_dom_homo), ecog(:,:,is_sub_homo), ecog(:,:,is_null_homo),...
       'SigPts', is_sigpts, 'Title', fig_title);
