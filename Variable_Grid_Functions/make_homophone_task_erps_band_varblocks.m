...
if isempty(relevant_blocks)
    error(['No Blocks Detected, for load_blocks = "' load_blocks '"'])
end
...
extraneous_chans = find(isnan(squeeze(ecog_targets(:,1,1)))); % There are non real channels included
ecog_primes(extraneous_chans,:,:) = [];
ecog_targets(extraneous_chans,:,:) = [];
bad_chans = setdiff(bad_chans, extraneous_chans);
