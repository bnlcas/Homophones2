function ERPs = Make_unified_ERP_struct_vargrid(evnt_dir, varargin)
...
% 
% The key feature of this function (and difference between this and the
% previous function is that it is designed to handle data
% from non-grid electrodes
% 
%
% Inputs:
...
% Variable Input:
% 
% 1 - band_indecies: list of bands (from set of 1:40)
% Setting this will engage a separate analysis track from the default High Gamma only
% 
% 2 - load_non_grid: boolean (default: false) if true, will attempt to find
% non-grid electrodes
%
%
%
...
%% Check for default
load_specifed_bands = false;
if length(varargin) > 0
    if ~isempty(varargin{1})
        band_indecies = varargin{1};
        load_specifed_bands = true;
    end
end

load_non_grid = false;
if length(varargin)>1 && ~isempty(varargin{2})
    load_non_grid = varargin{2};
end
...
%    isi_full = [isi_full isi];
    ...
    %% Key Mod - Tract High Gamma vs Arbitrary Band Data separately:
    if load_non_grid
        load_blocks = 'nongrid';
        bad_times = 'badTimeSegments_nongrid.mat';
        bad_chans = 'badChannels_nongrid.txt';
    else
        load_blocks = 'grid';
        bad_times = 'badTimeSegments_grid.mat';
        bad_chans = 'badChannels_grid.txt';
    end
    
    if ~load_specifed_bands
        if ~load_non_grid
            ERPs_block = make_homophone_task_erps_gen(evnt);
        else
            ERPs_block = make_homophone_task_erps_band_varblocks(evnt, [], load_blocks, bad_times, bad_chans);
        end
    else
        if ~load_non_grid
            ERPs_block = make_homophone_task_erps_band_varblocks(evnt,band_indecies, load_blocks, bad_times, bad_chans);
        else
            ERPs_block = make_homophone_task_erps_band_varblocks(evnt, band_indecies, load_blocks, bad_times, bad_chans);
        end
    end
...
ERPs.is_non_grid = load_non_grid;
