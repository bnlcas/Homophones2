...
%subj = 'EC131';
%blocks = {'B19', 'B21', 'B26', 'B31', 'B35', 'B42', 'B49', 'B62'};
%blocks = {'B18', 'B42', 'B54', 'B59', 'B63', 'B66'};
%root_dir = '/Users/changlab/Documents/data/EC123';  % Subject directory described above
...
if create_evnt_structs
    ...
    homophone_process_data(subj, root_dir, missing_blocks); % Generates evnt Strucutres
end
...
for k = 1:length(band_names);
    ...
    ERPs.BadChans(ERPs.BadChans>size(ERPs.ecog_targets,1)) = [];
    ...
    if plot_erps
        ...
        is_mouse = is_good & strcmpi(ERPs.target_names,'shake'); % target homophone is mouse

        PlotECogGrid_std(ERPs, true, [-0.5 1], ERPs.ecog_targets(:,:,is_rel & is_mouse), ERPs.ecog_targets(:,:,is_unr & is_mouse))
...
for j=1:length(band_names);
    load([root_dir filesep 'ecog_data' filesep 'ERP_structure' filesep subj '_ERPs_' band_names{j} '.mat'])
    is_good = (ERPs.is_good_trial == 1);    % Trials with no bad time segments
 is_sub = (ERPs.is_related_subordinant == 1) & is_good;  % subordinate sense primed
 is_dom = (ERPs.is_related_dominant == 1) & is_good; % dominante sense primed
 is_rel = is_sub | is_dom;                       % Related prime
 is_unr = is_good & ~is_sub &~is_dom;            % Rando prime
    ...
    PlotECogGrid_std2(ERPs, true, [-0.5 1], ERPs.ecog_targets(:,:,is_dom), ERPs.ecog_targets(:,:,is_sub), ERPs.ecog_targets(:,:,is_unr))
%    title([band_names{j} ' band all']);
    mTextBox = uicontrol('style','text');
    set(mTextBox,'String',[subj ' ' band_names{j} ' all'], 'Position', [20 412 100 9]);
    print([root_dir filesep 'figures' filesep subj '_' band_names{j} '_dom_sub_unrel_all'], '-dpng');

    homophones = unique(ERPs.target_names);

    for k=1:length(homophones)
        homophone=homophones{k};
        is_homo = is_good & strcmpi(ERPs.target_names,homophone);
        PlotECogGrid_std2(ERPs, true, [-0.5 1], ERPs.ecog_targets(:,:,is_dom & is_homo), ERPs.ecog_targets(:,:,is_sub & is_homo), ERPs.ecog_targets(:,:,is_unr & is_homo));
        mTextBox = uicontrol('style','text');
        set(mTextBox,'String',[subj ' ' band_names{j} ' ' homophone], 'Position', [20 412 100 9]);
        print([root_dir filesep 'figures' filesep subj '_' band_names{j} '_dom_sub_unrel_' homophone], '-dpng');
    end
